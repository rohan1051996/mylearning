    ...
    <div class="table-wrapper">
      <div class="table-title">
        <div class="row">
          ...
          <div class="col-sm-6">
            <a href="add.php" class="btn btn-success"><i class="material-icons">&#xE147;</i> <span>Add New city</span></a>
          </div>
        </div>
      </div>
      <table class="table table-striped table-hover">
        ...
        if ($result->num_rows > 0) {
          while($row = $result->fetch_assoc()) {
            $resxx = $conn->query("SELECT * FROM `state` WHERE `id` = '".$row['state_id']."'");
            $state = $resxx->fetch_all(MYSQLI_ASSOC);
